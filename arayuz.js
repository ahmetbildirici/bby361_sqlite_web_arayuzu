...
class SQLiteWebJS {
  ...
  showToTableContainer(container) {
    ...
    document.getElementById('table-name').innerHTML =
      (this.currentStructure.type == Structure.VIEW ? 'Görüntü: ' : 'Tablo: ') +
  ...
  executeQuery(options, containerObj = null) {
    ...
    if (options.result.rows.length) {
      if (options.result.rows.length == 1) resultExecute.message = '1 satır';
      else resultExecute.message = options.result.rows.length + ' satır';
    } else {
  ...
  insertView(tableInfo) {
    ...
    ['Sütun', 'Tür', 'NULL', 'Değer'].forEach(function (c) {
        ...
        if (document.getElementById('file-input').value != '') {
          ...
          setTimeout(function () {
            self.readSingleFile(e, ReadModeEnum.ARRAY_BUFFER, function (data) {
              ...
              self.resetMessage();
              self.setMessage('arayuz-editor-valid', 'Veritabanınız yüklendi.');
            });
          }, 1);
        }
      },
    ...
    document.getElementById('update').addEventListener(
      ...
      function (e) {
        ...
        setTimeout(function () {
          ...
          self.setMessage(
            'arayuz-editor-valid',
            'Tablo, görüntü, indeks ve sequence listesi güncellendi.'
          );
          setTimeout(function () {
